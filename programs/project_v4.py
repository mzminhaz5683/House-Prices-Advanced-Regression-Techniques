...
'''
print(all_data.shape)
final_all_data = pd.get_dummies(all_data).reset_index(drop=True)
print(final_all_data.shape)

X = final_all_data.iloc[:len(y_train), :]
X_sub = final_all_data.iloc[len(X):, :]

print('X', X.shape, 'y', y_train.shape, 'X_sub', X_sub.shape)
